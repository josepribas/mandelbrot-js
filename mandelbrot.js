onmessage = function(message) {
    ...
    postMessage({ tile, imageData, max, min });
};
...
class Tile {
    ...
    static *tiles(width, height, numRows, numColumns) {
        let columnWidth = Math.ceil(width / numColumns);
        ...
        for (let row = 0; row < numRows; row++) {
            ...
            for (let col = 0; col < numColumns; col++) {
                let tileWidth = (col < numColumns - 1) ? 
                                columnWidth : 
                                width - columnWidth * (numColumns - 1);
